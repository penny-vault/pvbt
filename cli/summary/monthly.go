...
func renderMonthlyReturns(builder *strings.Builder, monthly monthlyReturns) {
	...
	// Header row.
	hdr := padRight("", yearLabelCol)
	for _, month := range monthHeaders {
		hdr += padLeft(tableHeaderStyle.Render(month), monthCol)
	}

	hdr += padLeft(tableHeaderStyle.Render("Year"), monthCol)
