...
- The Schwab broker (`broker/schwab`) enables live equity trading through the Charles Schwab Trader API. It implements the full Broker interface and GroupSubmitter, supporting market, limit, stop, and stop-limit orders along with native bracket and OCO order groups via Schwab's nested order strategies. Authentication uses OAuth 2.0 with automatic token refresh and file-based token persistence so that credentials survive process restarts. Fills arrive in real time through Schwab's WebSocket streaming API, with automatic reconnection and missed-fill recovery.
