...
- Charles Schwab is now available as a live broker (`broker/schwab`), joining tastytrade and Alpaca as the third supported brokerage. Strategies can trade equities with market, limit, stop, and stop-limit orders, bracket and OCO order groups, dollar-amount orders, and tax lot selection. On first connection the broker walks you through Schwab's OAuth login in your browser and persists the credentials to disk so subsequent runs reconnect automatically. Fills stream in real time and recover gracefully from network interruptions.
