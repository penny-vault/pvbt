...
- Tax optimization middleware harvests losses automatically, tracks wash sales with IRS-compliant basis adjustment, supports configurable lot selection (FIFO, LIFO, highest-cost), and swaps to correlated substitute assets to avoid wash sale windows; the `tax.TaxEfficient` profile bundles these into a single middleware chain
- A `TaxAware` interface provides tax-lot-level access for middleware without exposing internals through the read-only `Portfolio` interface; the middleware type-asserts to `TaxAware` for wash sale history, unrealized lots, and realized gains
- Configurable lot selection lets accounts choose FIFO, LIFO, or highest-cost-first when selling positions, set as an account-wide default via `WithDefaultLotSelection` or per-order via `WithLotSelection`; tax lots now carry unique IDs that persist through SQLite round-trips
- Substitution mapping presents a logical portfolio view to strategies when the tax middleware swaps an asset for a correlated substitute; `Holdings()`, `ProjectedHoldings()`, and `ProjectedWeights()` show the original asset while the transaction log reflects reality
