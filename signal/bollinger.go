...
// three metrics: BollingerUpperSignal, BollingerMiddleSignal, BollingerLowerSignal.
func BollingerBands(ctx context.Context, assetUniverse universe.Universe, period portfolio.Period, numStdDev float64, metrics ...data.Metric) *data.DataFrame {
	...
	if df.Len() < 2 {
		return data.WithErr(fmt.Errorf("BollingerBands: need at least 2 data points, got %d", df.Len()))
	}
	...
	result, err := data.MergeColumns(middle, upper, lower)
	if err != nil {
		return data.WithErr(fmt.Errorf("BollingerBands: %w", err))
	}
