...
The `asset.Asset` struct only carries `CompositeFigi` and `Ticker`. The PostgreSQL `assets` view has rich metadata (sector, industry, asset type, listing dates, etc.) that strategies need for filtering. For example, NCAVE needs to exclude financial-sector stocks but has no way to access sector data through the current Asset type.

## Design

### Asset struct

Extend `asset.Asset` with metadata fields and introduce four string-typed enums: `AssetType`, `Sector`, `Industry`, and `Exchange`.
...
Raw database values are normalized at scan time:

| Raw values | Normalized constant |
|---|---|
| NYSE, XNYS, NYSE ARCA, NYSE MKT, ARCX, XASE, AMEX | `ExchangeNYSE` |
| NASDAQ, XNAS, NMFQS | `ExchangeNASDAQ` |
| BATS | `ExchangeBATS` |
| FRED | `ExchangeFRED` |

A `NormalizeExchange(raw string) Exchange` function in the `asset` package handles this mapping.
...
All four enum types and their constants live in `asset/asset.go` alongside the struct. The industry constants are long but belong with the type definition.

### Database read paths

**PVDataProvider** (PostgreSQL):

`LookupAsset()` and `Assets()` expand their SELECT to include `name, asset_type, primary_exchange, sector, industry, sic_code, cik, listed, delisted`. All columns in the view are nullable, so scanning uses `sql.Null*` types, converting to zero values when NULL. The `asset_type`, `sector`, and `industry` columns scan as strings and are cast to their enum types. The `primary_exchange` column is scanned as a string and passed through `asset.NormalizeExchange()` to map raw exchange codes to the coarse enum.

**SnapshotProvider** (SQLite):

Same SELECT/scan expansion against the local `assets` table.

`IndexMembers()` and `RatedAssets()` currently construct Assets from their own tables with only figi+ticker. These queries gain a JOIN to the `assets` table to populate the full metadata.

### Snapshot schema

The `assets` CREATE TABLE expands to:
...
### SnapshotRecorder write path

`recordAssets()` expands its INSERT to write all 11 fields.

### Index state / changelog

`loadIndexState()` and `index_state.go` construct Assets from JSON/changelog data that only has figi+ticker. These get zero-value metadata, which is acceptable -- the assets are available in the main assets table for enrichment if needed.

### Sentinel values

`CashAsset`, `EconomicIndicator`, and `Factor` stay as-is. Zero values for the new fields are correct for synthetic sentinels.

### Testing

- Update `snapshot_recorder_test.go` to construct Assets with new fields and verify round-trip through SQLite.
- Update `snapshot_provider_test.go` to verify new fields read back correctly.
- Existing tests that construct `Asset{CompositeFigi: "...", Ticker: "..."}` still compile -- new fields default to zero values.

### Scope boundary

This spec covers extending the Asset struct and plumbing metadata through the data layer. It does not cover:
- Adding filtering helpers (e.g., `asset.IsSector(SectorFinancialServices)`) -- strategies can compare directly.
- Changing the `AssetProvider` interface signature -- returns the same type, just with more populated fields.
- Modifying the engine, DataFrame, or strategy interface.
