...
3. **Set up the shadow environment.** Create a fresh `SimulatedBroker`. Wire it
   to the clone via `clone.SetBroker(shadowBroker)`. Save and restore
   `eng.currentDate` after the prediction run.
...
1. Determines the fill frequency from `df.Frequency()`.
2. Generates timestamps from `df.End() + 1 frequency step` through `targetDate`,
   using the frequency to determine spacing (Daily = calendar days, Tick = seconds,
   Weekly = 7 days, Monthly = 1 month, etc.).
...
between steps), perfData (shallow), benchmark, riskFree, metadata (deep copy
of map), annotations (copy of slice). Tax lots, registered metrics, and the
transaction log are copied shallowly (the prediction run may append to the
transaction log, but that's fine since it's the clone's copy).
