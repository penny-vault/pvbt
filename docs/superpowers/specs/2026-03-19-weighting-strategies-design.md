...
Full iterative optimization using the Spinu (2013) formulation via Newton's method:

1. Compute the covariance matrix `C` over the lookback window.
...
4. Compute gradient of the objective `sum((rc_i - 1/N)^2)` with respect to weights.
5. Update weights via Newton step, project onto the simplex (non-negative, sum to 1).
6. Converged when `max(|rc_i - 1/N|) < 1e-10`.
7. Maximum 1000 iterations. Returns best result found after hitting the cap and logs a warning via zerolog if not converged.

Uses `gonum.org/v1/gonum/mat` for matrix operations (already a project dependency via gonum).
